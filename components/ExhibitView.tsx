...
export default function ExhibitView() {
  ...
  if (x.phase === "intro") {
    return (
      <div className="absolute left-[140px] top-[60px] flex h-[960px] w-[1640px] flex-col items-center justify-center">
        <h2 className="fade-up relative z-10 text-center text-[46px] font-light leading-[1.4] tracking-[-1.4px] text-white">
          단어별 물 사용량을
          <br />
          체감하세요
        </h2>
        <p className="relative z-10 mt-[14px] text-[19px] tracking-[-0.75px] text-label">
          한 글자를 적을 때마다 {ML_PER_CHAR}mL — 컵 한 잔은 {EXHIBIT_LIMIT}mL입니다
        </p>
        {/* 컵 영상은 정사각 캔버스 안에서 실제 컵이 세로 45%(위 여백 25%)만 차지한다.
            여백만큼 음수 마진으로 당겨 눈에 보이는 간격을 맞추고, 캔버스가 --bg로 꽉 찬 불투명
            사각형이라 겹치는 텍스트는 z-10으로 위에 올린다 (배경색이 같아 겹침이 보이지 않는다). */}
        <div className="-mb-[238px] -mt-[186px] size-[900px] shrink-0">
          <SendCupVideo stage={1} />
        </div>
        <button
          type="button"
          onClick={x.start}
          className="relative z-10 mt-[10px] h-[68px] w-[200px] shrink-0 cursor-pointer rounded-[73px] border-4 border-main text-[24px] font-light tracking-[-0.72px] text-white transition-colors hover:bg-main/20"
        >
          시작하기
        </button>
      </div>
  ...
  /* ---------- 입력 / 결과 ----------
     컵·게이지는 두 화면에서 같은 자리에 두고 위아래 문구와 하단만 바꾼다.
     그래야 결과로 넘어갈 때 컵이 그 자리에서 줄어드는 모습이 그대로 보인다. */
  return (
    <div className="absolute left-[140px] top-[60px] flex h-[960px] w-[1640px] flex-col items-center justify-center">
      {/* 상단 문구 — 높이를 고정해 결과 문구로 바뀌어도 컵이 위아래로 흔들리지 않는다 */}
      <div className="relative z-10 flex h-[120px] shrink-0 flex-col items-center justify-end">
        {showResult ? (
          <>
            <p key={`q-${x.sends}`} className="fade-up text-[20px] tracking-[-0.8px] text-label">
              “<span className="text-white">{r.text}</span>”
            </p>
            <h2 key={`h-${x.sends}`} className="fade-up mt-[10px] text-center text-[40px] font-light tracking-[-1.2px] text-white">
              {x.empty ? (
                "물이 모두 사라졌습니다"
              ) : (
                <>
          ...
          <>
            <h2 className="text-center text-[34px] font-light tracking-[-1px] text-white">단어를 적어보세요</h2>
            <p className="mt-[10px] text-[17px] tracking-[-0.7px] text-label">
              보내기를 누르면 그 단어에 쓰인 물의 양을 보여드려요
            </p>
          </>
        )}
      </div>

      {/* 컵 영상 캔버스 안에서 내용은 세로 25%~90% 구간에만 있다 (컵 70%까지 + 그림자가 돌며 90%까지).
          위 여백은 25%만큼, 아래 여백은 그림자가 가장 길게 뻗는 90% 이후만 당겨서
          어느 단계에서도 그림자가 게이지에 닿지 않게 한다. */}
      <div className="-mb-[54px] -mt-[140px] size-[620px] shrink-0">
        <SendCupVideo stage={x.stage} />
      </div>

      {/* 남은 물 — 보내기 전에는 그대로, 결과에서 한 번에 줄어든다 */}
      <div className="relative z-10 mt-[6px] flex w-[900px] shrink-0 flex-col gap-[12px]">
      ...
      {/* 하단 — 입력 바 또는 결과 계산식 + 버튼. 같은 높이를 차지해 레이아웃이 고정된다 */}
      <div className="relative z-10 mt-[22px] flex h-[140px] w-[900px] shrink-0 flex-col items-center">
        {showResult ? (
          <>
            <p key={`c-${x.sends}`} className="fade-up text-[18px] tracking-[-0.7px] text-label">
              {r.cost.chars}자 × {ML_PER_CHAR}mL = <span className="font-semibold text-white">{r.cost.ml}mL</span>
              {x.empty && ` · 컵 한 잔을 ${x.sends}번 만에 비웠습니다`}
            </p>
          ...
          <>
            ...
            <div className="flex h-[74px] w-full items-center gap-[14px] rounded-[16px] bg-gray-box px-[24px]">
              <input
                value={x.input}
                autoFocus
